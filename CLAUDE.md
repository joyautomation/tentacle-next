...
While on the `feature/mantle` branch, also run `bash scripts/deploy-remote.sh compulab@iot-gate-imx8.tail913f1.ts.net` after each completed chunk so the live edge picks up the same code as the dev containers. Run it in the background — the cross-compile + transfer takes a few minutes.
