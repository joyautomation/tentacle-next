...
func (m *Module) mountGitServerAPI(r chi.Router) {
	...
	r.Delete("/fleet/nodes/{group}/{node}", m.handleDeleteFleetNode)
	r.Get("/fleet/nodes/{group}/{node}/services", m.handleGetFleetNodeServices)
	r.Put("/fleet/nodes/{group}/{node}/services/{name}", m.handlePutFleetNodeService)
	r.Delete("/fleet/nodes/{group}/{node}/services/{name}", m.handleDeleteFleetNodeService)
}
