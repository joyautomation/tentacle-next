...
func enableForwarding() error {
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		slog.Error("nftables: failed to enable ip forwarding", "error", err, "output", string(out))
		return err
	}
	slog.Info("nftables: IPv4 forwarding enabled")

	// Persist to disk.
	confPath := filepath.Join(sysctlConfDir, sysctlConfFile)
	if err := os.MkdirAll(sysctlConfDir, 0755); err != nil {
		slog.Warn("nftables: failed to create sysctl.d directory", "error", err)
		return err
	}
	if err := os.WriteFile(confPath, []byte(sysctlContent), 0644); err != nil {
		slog.Warn("nftables: failed to persist sysctl config", "path", confPath, "error", err)
		return err
	}
	slog.Info("nftables: sysctl config persisted", "path", confPath)
	return nil
}
