...
const defaultServiceType = "profinet"

// Module implements the module.Module interface for the PROFINET IO Device.
type Module struct {
	moduleID string
	manager  *Manager
	stopHB   func()
...
// New creates a new PROFINET module.
...
func (m *Module) Start(ctx context.Context, b bus.Bus) error {
	...
	for _, bucket := range []string{topics.BucketHeartbeats, topics.BucketServiceEnabled, topics.BucketProfinetConfig} {
		if err := b.KVCreate(bucket, topics.BucketConfigs()[bucket]); err != nil {
			m.log.Warn("profinet: failed to create bucket", "bucket", bucket, "error", err)
		}
	}
	...
	if data, _, err := b.KVGet(topics.BucketServiceEnabled, m.moduleID); err == nil {
		...
		if json.Unmarshal(data, &state) == nil {
			if !state.Enabled {
				m.log.Info("profinet: service disabled via KV", "moduleId", m.moduleID)
			}
		}
	}
	...
	enabledSub, err := b.KVWatch(topics.BucketServiceEnabled, m.moduleID, func(key string, value []byte, op bus.KVOperation) {
		if op == bus.KVOpDelete {
			m.log.Info("profinet: enabled key deleted, defaulting to enabled")
			return
		}
		var state types.ServiceEnabledKV
		if json.Unmarshal(value, &state) == nil {
			m.log.Info("profinet: enabled state changed", "enabled", state.Enabled)
		}
	})
	if err != nil {
		m.log.Warn("profinet: failed to watch service_enabled KV", "error", err)
	} else {
	...
	shutdownSub, _ := b.Subscribe(topics.Shutdown(m.moduleID), func(subject string, data []byte, reply bus.ReplyFunc) {
		m.log.Info("profinet: received shutdown command via bus")
		_ = m.Stop()
		os.Exit(0)
	})
	m.subs = append(m.subs, shutdownSub)

	m.log.Info("profinet: service running", "moduleId", m.moduleID)
